...
function main($a) {
    $service  = get_client();
    if ($a) {$funcName = 'callSomeMagicMethod_1';}

    try {
        printf("Calling\n");
        flush();
        list($pets, $scores, $numfound) = call_user_func_array(array($service, $funcName), array($xxxxxxx, $cccccc, $dddd, $ggggggg, $aaaaaaaaaaa));
